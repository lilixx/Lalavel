...
<form class="form-horizontal" role="form" method="POST" action="{{ route('entidades.update', $huespede->id ) }}" enctype="multipart/form-data">
    ...
    <div class="col-sm-8">
      <select  class="form-control input-sm" name="paise_id">
       <option value="0" selected="true" disabled="true">Seleccione un país</option>
        @foreach ($pais as $pa)
           <option value="{{ $pa->id }}"  @if($pa->id==$huespede->paise_id)
